...
class SerialWorker(QRunnable):
    ...
    def readData(self):
        ...
        if(dataArray[0]==10 and dataArray[lastIndex]==11):
            ...
            if (p == 40): # 60 seconds
                ...
                self.delta=50
                self.n_peaks_lp1, self.c = self.find_peaks(zData_lowpass, self.threshold_lp, self.delta)
                ...
                self.delta1=50
                self.n_peaks_bp , self.c= self.find_peaks(zData_bandpass, self.threshold_bp, self.delta1)
                ...
                self.threshold_sm=self.calibration_threshold(zData_smoothed1)
                self.n_peaks_sm1, self.c = self.find_peaks(zData_smoothed1, self.threshold_sm, self.delta)
                
                print('numero picchi sm1',self.n_peaks_sm1)

                self.threshold_wi=self.calibration_threshold(zData_windowed)
                self.n_peaks_w , i_peaks_w= self.find_peaks(zData_windowed, self.threshold_wi, self.delta)
                self.makesum=0.0
                ...
                '''
                plt.figure(1)
                plt.plot(zData_lowpass, label = 'zData_lowpass')
                plt.plot(self.n_peaks_lp1,zData_lowpass[self.n_peaks_lp1], "x")
                ...
                plt.plot(i_peaks_w,zData_windowed[i_peaks_w], "x")
    ...
    def fast_fourier_transformation(self, signal_array, sample_rate):
        ...
        xf=fftfreq(N,T)[:N//2]
        plt.semilogy(xf[1:N//2], 2.0/N * yf[1:N//2], '-b')
        plt.semilogy(xf[1:N//2], 2.0/N * ywf[1:N//2], '-r')
...
class MainWindow(QMainWindow):
    ...
    def drawGeneralGraph(self):
        ...
        """
        global xData, yData, zData, xData_g, yData_g, zData_g, zData_lowpass, zData_bandpass, zData_BP_FT, zData_smoothed, sum_data
